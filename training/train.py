...
def get_optimizer(model, tcfg: TrainConfig, phase=0):
    if isinstance(model, KVAE):
        param_groups = [
            ...
            {"params": model.alpha_net.parameters(),        "lr": tcfg.learning_rate * 0.5},
...
def train_epoch(model, loader, optimizer, cfg, tcfg, epoch, mask, device, phase=1):
    ...
    for batch in loader:
        ...
        S_pred, Q, alpha_seq) = model(ball_seq, obstacle_img, u_seq=u_seq, mask=current_mask, epoch=epoch)
...
def pretrain_epoch(model, loader, optimizer, cfg, tcfg, epoch, device):
    ...
    for batch in loader:
        ...
        a_dist, a_seq, a_smooth, a_pred_smooth,
        z_dist, z_smooth, P_smooth, z_pred, P_pred,
        S_pred, Q, _) = model(ball_seq, obstacle_img, u_seq=u_seq, phase=0)
...
def eval_epoch(model, loader, cfg, tcfg, epoch, mask, device):
    ...
    for batch in loader:
        ...
        a_dist, a_seq, a_smooth, a_pred_smooth,
        z_dist, z_smooth, P_smooth, z_pred, P_pred,
        S_pred, Q, alpha_seq) = model(ball_seq, obstacle_img, u_seq=u_seq, mask=current_mask)
...
def train(model, train_loader, val_loader, cfg, sim_cfg, tcfg, device, logger):
    ...
    for epoch in range(1, 201):
        ...
        if epoch < 50:
            ...
            steps = int(4 * (epoch-100) / 50)
            mask = make_mask(B, T, device, steps, tcfg.p_mask, randomize_start=True)
        else:
            mask = mask = make_mask(B, T, device, 4, tcfg.p_mask, randomize_start=False)

        mask_val = make_mask(B, T, device, tcfg.free_running_steps, p_mask=0, randomize_start=False)

        train_terms = train_epoch(model, train_loader, optimizer, cfg, tcfg, epoch, mask, device, phase=1)
