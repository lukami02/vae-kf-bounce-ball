...
class KalmanFilter(nn.Module):
    ...
    def forward(self, a_seq, alpha_net, h_obs, A_matrices, C_matrices, B_matrices=None, u_seq=None, mask=None, epoch=100):
        ...
        for j in range(self.cfg.num_matrices):
            ...
            a_next_per_expert.append(a_pred_j)  # [B, dim_a]
        a_next_all = torch.cat(a_next_per_expert, dim=-1)  # [B, K * dim_a]
        
        alpha_k, gru_state = alpha_net(a_prev, h_obs, z.detach(), gru_state, temp=self.cfg.get_temperature(epoch), a_next_all=a_next_all)
        ...
        for k in range(T):
            ...
            alpha_k, gru_state = alpha_net(a_for_alpha, h_obs, z_filt, gru_state, temp=self.cfg.get_temperature(epoch), a_next_all=a_next_all)
